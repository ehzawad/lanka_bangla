...
function send(message){
    ...
    x = xhr.open('GET', 'http://192.168.10.78:5025/message?sender='+sender+'&output=' + encodeURIComponent(userInput), true);
    ...
    xhr.onreadystatechange = function() {
      if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
        ...
        console.log(data[0].bt, "********************")
        setBotResponse(data[0].bt)
        // data[0].audio = "";
...
function actionTrigger() {
    $.ajax({
        url: `http://192.168.10.78:5005/conversations/${sender_id}/execute`,
        type: "POST",
...
function customActionTrigger() {
    $.ajax({
        url: "http://192.168.10.78:5055/webhook/",
        type: "POST",
        contentType: "application/json",
        data: JSON.stringify({
