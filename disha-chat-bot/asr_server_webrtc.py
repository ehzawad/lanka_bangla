...
vosk_interface = os.environ.get('VOSK_SERVER_INTERFACE', '58.65.228.98')
...
PREV_TEXT = ""
TTS_IP = "http://192.168.10.97:7070"
...
async def message(request):
    ...
    # print(post_data)
    # req = rqst.Request("http://58.65.228.98:5001/webhooks/rest/webhook", data=post_data)
    req = rqst.Request("http://192.168.10.78:5055/webhooks/rest/webhook", data=post_data)
    ...
    url=f"{TTS_IP}/api/tts?text="+str(br)+"&speaker_id=&style_wav=&language_id="
    req = rqst.Request(url)
    post_resp = rqst.urlopen(req)
    audio_data = post_resp.read()
    audio_base64 = base64.b64encode(audio_data).decode("utf-8")

    dat = [{"text": bot_response, "audio": audio_base64, "recipient_id": sender}]
...
if __name__ == '__main__':
    ...
    #web.run_app(app, port=vosk_port, host="192.168.10.91", ssl_context=ssl_context)
    #web.run_app(app, port=vosk_port, host="noctest.gplex.net", ssl_context=ssl_context)
    web.run_app(app, host="58.65.228.98", port=5025)
